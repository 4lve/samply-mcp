...
└── analysis/
    ...
    ├── samples.rs       # Profile-relative ranges and sample-equivalent CPU time
    ├── source.rs        # Exclusive instruction/source-line aggregation
...
- `profile_flamegraph` — collapsed stack format text, optionally focused or aggregated by thread name prefix (params: path, thread/tid/thread_index or thread_name_prefix, focus_function)

Sample-based tools also accept optional inclusive `start_time_ms`/`end_time_ms` bounds relative to the first observed profile sample, and return the clamped `effective_range`.
