...
Use `c2c_top_cachelines` when looking for one heavily shared line, then pass its `cacheline_address` to `c2c_cacheline_detail`. Use `c2c_top_access_sites` when one instruction—such as a lock operation over many heap objects—may be fragmented across many lines; pass its stable `site_id` or instruction address to `c2c_access_site_detail` for cacheline, offset, co-accessor, thread, CPU, and callchain attribution. Access-site rows expose every unmodified `PERF_SAMPLE_IP` represented by the group, the `PERF_RECORD_MISC_EXACT_IP` coverage, and an exact `PERF_SAMPLE_DATA_SRC` distribution alongside the decoded operation, memory level, HITM, peer, and lock statistics. Data-address detail includes any containing MMAP/MMAP2 region recorded by perf, but ordinary perf c2c captures do not include heap-allocation provenance. All four tools accept `pid`, `tid`, `thread_name_prefix`, and profile-relative time filters. Source resolution uses the binary paths recorded in the capture, so keeping the exact binaries and their debug information available gives the best results. The parser is implemented in Rust and reads `perf.data` directly; it does not shell out to `perf c2c report`.

Profile summaries distinguish all sample records, memory sample records, usable memory samples with a nonzero data address, and samples matched by the requested filters. They also include capture-wide statistics and decoded memory-level aggregates; ranking tools include the same aggregates after request filters. A latency sort is rejected when no matched sample has instruction-latency data, and summaries warn when every sample has unit weight. The availability and precision of HITM, peer, physical-address, and instruction-latency fields depend on the processor, kernel, and selected `perf c2c` event. Cacheline size is currently assumed to be 64 bytes and is reported as a warning in every result.
